...
<%@page import="com.islora.lock.wx.server.db.dao.LcAuthCardDao"%>
...
<%@page import="com.islora.lock.wx.server.db.entity.LcAuthCard"%>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><%=user.getUsername() %>的卡授权列表</h3>
              <div class="box-tools">
              <div class="btn-group">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-hover">
                ...
                <tbody>
                <%
                List<LcAuthCard> auths=new LcAuthCardDao().findByOwnerId(user.getId()); 
                ...
                if(auths!=null&&auths.size()>0){
                	for(LcAuthCard auth:auths){
                		...
                		if("time".equals(auth.getCtype())){
                			tt="M1卡时间授权";
                			client=userMap.get(auth.getClientId());
                		}else if("sntime".equals(auth.getCtype())){
                			tt="身份证时间授权";
                		}
                %>
                <tr>
                  <td><%=client!=null?client.getPhone():auth.getReaderNo()%></td>
                  <td><%=tt %></td>
                  <td><%=community!=null?community.getName():"" %></td>
                  <td><%=build!=null?build.getName():"" %></td>
                  <td><%=ap!=null?ap.getName():"" %></td>
                  <td><%=client!=null?auth.getReaderNo():""%></td>
                  <td><%=auth.getStartTime() %></td>
                  <td><%=auth.getEndTime() %></td>
                  <td>
                  <table>
                  <tr>
                  <td> <button onclick="authEdit('<%=auth.getId() %>','<%=client!=null?client.getPhone():auth.getReaderNo() %>')"  data-toggle="modal" data-target="#modal-edit-auth" type="button" class="btn btn-block btn-info">修改</button></td>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <tr>
                	<td>身份证号：</td>
                	<td><input type="text" id="add_auth_cardsn" name="phone" class="form-control" placeholder="输入客户身份证"></td>
                </tr>
                <tr>
                	<td>起始时间：</td>
